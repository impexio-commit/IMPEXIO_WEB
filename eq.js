/* ============================================================
   ...
   async function loadRecordsFromAPI() {
     try {
       ...
       console.error('Failed to load EQ records:', err);
     }
   }
   ...
   async function saveRecord() {
     ...
     const payload = {
       ...
       deliveryTime: gv('f_delivery_time'),
       shipmentType: gv('f_shipment_type'),
       paymentTerms: gv('f_payment_terms'),
       validity: gv('f_validity'),
       packaging: gv('f_packaging'),
       containerSize: gv('f_container_size'),
       packedDim: gv('f_packed_dim'),
       innerPack: gv('f_inner_pack'),
       packedWeight: gv('f_packed_weight'),
       masterPack: gv('f_master_pack'),
       sample: gv('f_sample'),
       specialInst: gv('f_special_inst'),
       otherDesc: gv('f_other_desc'),
   ...
   async function editRecord(id) {
     try {
       ...
       sv('f_quotno',   rec.quotNo);
       sv('f_date',     rec.quotDate?.split('T')[0] || '');
       sv('f_product',  rec.product);
       sv('f_buyer',    rec.buyer);
       sv('f_country',  rec.country);
       sv('f_pol',      rec.pol);
       sv('f_pod',      rec.pod);
       sv('f_incoterms', rec.incoterms);
       sv('f_final_dest', rec.finalDest);
       sv('f_delivery_time', rec.deliveryTime);
       sv('f_shipment_type', rec.shipmentType);
       sv('f_payment_terms', rec.paymentTerms);
       sv('f_validity', rec.validity);
       sv('f_packaging', rec.packaging);
       sv('f_container_size', rec.containerSize);
       sv('f_packed_dim', rec.packedDim);
       sv('f_inner_pack', rec.innerPack);
       sv('f_packed_weight', rec.packedWeight);
       sv('f_master_pack', rec.masterPack);
       sv('f_sample', rec.sample);
       sv('f_special_inst', rec.specialInst);
       sv('f_other_desc', rec.otherDesc || '( Testing Charges, Inspection Charges and Special Packaging Charges will be extra as applicable )');
       ...
       if (rec.rows && rec.rows.length > 0) {
         rec.rows.forEach(row => {
           ...
           const id2 = rowCount;
           const sv2 = (k, v) => {
             const el = document.getElementById(`r_${k}_${id2}`);
             if (el) el.value = v || '';
           };
           sv2('desc', row.desc);
           sv2('hs', row.hs);
     ...
     list.innerHTML = items.map(rec => `
       <div class="ql-card ${editingId===rec.id?'active':''}" id="eqcard_${rec.id}" onclick="editRecord(${rec.id})">
         ...
         <div class="ql-card-row">
           <span class="ql-card-date">${fmtDate(rec.quotDate)}</span>
           <span class="ql-card-val">${rec.totAmt||'0.00'}</span>
         </div>
   ...
   function printRecord() {
     calcTotals();
     doPrint({
       quotNo: gv('f_quotno'), quotDate: gv('f_date'),
       product: gv('f_product'), buyer: gv('f_buyer'), country: gv('f_country'),
       pol: gv('f_pol'), pod: gv('f_pod'),
       incoterms: gv('f_incoterms'), finalDest: gv('f_final_dest'),
       deliveryTime: gv('f_delivery_time'), shipmentType: gv('f_shipment_type'),
       paymentTerms: gv('f_payment_terms'), validity: gv('f_validity'),
       packaging: gv('f_packaging'), containerSize: gv('f_container_size'),
       packedDim: gv('f_packed_dim'), innerPack: gv('f_inner_pack'),
       packedWeight: gv('f_packed_weight'), masterPack: gv('f_master_pack'),
       sample: gv('f_sample'), specialInst: gv('f_special_inst'),
       otherDesc: gv('f_other_desc'), remarks: gv('f_remarks'),
   ...
   function doPrint(rec) {
     ...
     // Terms grid — uses camelCase API field names
     const termPairs = [
       ['Port of Loading',    rec.pol],
       ['Port of Discharge',  rec.pod],
       ['Inco Terms',         rec.incoterms],
       ['Final Destination',  rec.finalDest],
       ['Delivery Time',      rec.deliveryTime],
       ['Shipment Type',      rec.shipmentType],
       ['Payment Terms',      rec.paymentTerms],
       ['Quotation Validity', rec.validity],
       ['Packaging / Labeling',rec.packaging],
       ['Container Size',     rec.containerSize],
       ['Packed Dimension',   rec.packedDim],
       ['Total Inner Pack',   rec.innerPack],
       ['Packed Weight',      rec.packedWeight],
       ['Total Master Pack',  rec.masterPack],
     ];
     ...
     <div class="qp-doc">
       ...
       <div class="qp-meta">
         ...
         <div class="qp-meta-item"><span class="qp-meta-lbl">Date:&nbsp;</span><span class="qp-meta-val">${fmtDate(rec.quotDate)}</span></div>
       ...
       <table class="qp-tbl">
         ...
         <tbody>
           ...
           <tr class="qp-tot-row">
             ...
             <td>${rec.totQty||'0'}</td>
             <td></td>
             <td class="qp-tot-gold">${rec.totAmt||'0.00'}</td>
           </tr>
         </tbody>
       </table>
       ...
       <div class="qp-sample-box">
         <div class="qp-sample-row">
           ...
           <div class="qp-sample-item">
             ...
             <span class="qp-sample-val">${rec.specialInst||''}</span>
           </div>
         </div>
         <div class="qp-other-desc">Other Desc.: ${rec.otherDesc||''}</div>
       </div>
